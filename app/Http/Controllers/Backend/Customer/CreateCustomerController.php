...
use App\Mail\Register\RegisterCompanyPersonNatural;
...
class CreateCustomerController extends Controller
{
    ...
    {
        return view('backend.pages.customer.create-customers');
    }
    ...
    public function ulploadPictureCompany(Request $request)
    {
        $success = true;
        $urlFinale = null;
        DB::beginTransaction();
    ...
    public function uploadArchiveCustomerCompany(Request $request)
    {
    ...
    public function storeCreateCompany(Request $request)
    {
        ...
        try {
            ...
            $customer->position_id = $typePosition->id;
            $customer->company_category_id = $typeCompany->id;
            $customer->type_entitity_id = $typeEntity->id;
            $customer->save();
            ...
            for ($i = 0; $i < count($archives); $i++) {
                $customer->archive()->create([
                    ...
                    'archivable_type' => get_class($customer)
                ]);
            }
    ...
    public function storeCreatePersonaNaturalCustomer(Request $request)
    {
        ...
        try {
            ...
            Mail::to($user->email)->locale($language->code)->send(new RegisterCompanyPersonNatural($user->name, $password, $user->email));
        } catch (\Exception $exception) {
            $success = $exception->getMessage();
            DB::rollBack();
        }
    ...
    public function removedPictureCompany(Request $request)
    {
    ...
    public function removedArchiveCompany(Request $request)
    {
