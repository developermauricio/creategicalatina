...
@section('content')
    <section id="basic-datatable">
        <cards-statistcs-projects></cards-statistcs-projects>
        <div class="row">
            <div class="col-12">
                <div class="card p-2">
                    <table
        ...
        $(function () {
            setTimeout(() => {
                    var table = $('.datatables-all-projects-company').DataTable({
                        ...
                        "ajax": {
                            url: "{{route('api.backend.all.projects')}}",
                        },
                        "columns": [
                            {"data": "id"},
                            {
                                render: function (data, type, JsonResultRow, meta) {
                                    if (JsonResultRow.name === null) {
                                        ...
                                        return `<span class="label text-center font-weight-bold"><a href="/${window.lang}/admin-projects/${JsonResultRow.slug}">${JsonResultRow.name}</a></span>`;
                                    }
                                },
                            },
                            {
...

                            {
                                render: function (data, type, JsonResultRow, meta) {
                                    if (JsonResultRow.state === "1") {
                                        return '<div class="badge badge-pill badge-glow badge-warning">{{ __('titulo_estado_proyecto_revision') }}</div>'
                                    } else if (JsonResultRow.state === "2") {
                                        return `<div class="badge badge-pill badge-glow badge-primary">{{ __('titulo_estado_proyecto_borrador') }}</div>`;
                                    }
                                    else if (JsonResultRow.state === "3") {
                                        return `<div class="badge badge-pill badge-glow badge-success">{{ __('titulo_estado_proyecto_aprobado') }}</div>`;
                                    }
                                    else if (JsonResultRow.state === "4") {
                                        return `<div class="badge badge-pill badge-glow badge-info">En {{ __('titulo_estado_proyecto_desarrollo') }}</div>`;
                                    }
                                    else if (JsonResultRow.state === "5") {
                                        return `<div class="badge badge-pill badge-glow badge-danger">{{ __('titulo_estado_proyecto_rechazado') }}</div>`;
                                    }
                                    else if (JsonResultRow.state === "6") {
                                        return `<div class="badge badge-pill badge-glow badge-success">{{ __('Finalized') }}</div>`;
                                    }
                                },
                            },
                            ...
                            {
                                render: function (data, type, JsonResultRow, meta) {
                                    if (JsonResultRow.customer[0] === null) {
                                        return '<span class="label label-danger text-center" style="color:#F05E7D !important">{{ __('nigun_valor_defecto') }}</span>'
                                    } else {
                                        return `<span class="label text-center font-weight-bold">${JsonResultRow.customer[0].user.name} ${JsonResultRow.customer[0].user.last_name}</span>`;
                                    }
                                },
                            },
                            {
