                            ...
                            <table style="font-family: 'Montserrat',Arial,sans-serif; width: 100%;" width="100%"
                                ...
                                <tr>
                                    <td class="sm-px-24"
                                        ...
                                        <p style="font-weight: 600; font-size: 18px; margin-bottom: 0;">{{ __('backend/email/purchase-order-send-customer.titulo_hola') }} 👋</p>
                                        @if($name)
                                        ...
                                        <p style="margin: 25px 0 2px; text-align: justify">
                                            {{ __('backend/email/purchase-order-send-customer.titulo_se_generado_orden_pago') }} <strong>{{ $code }}</strong>, {{ __('backend/email/purchase-order-send-customer.titulo_para_proyecto') }} <strong>{{ $project }}.</strong>
                                        </p>
                                        @if($message)
                                        <h3 style="padding-top: 2rem">Mensaje:</h3>
                                        <p>
                                            {!! $message !!}
                                        </p>
                                        @endif
                                        <table style="font-family: 'Montserrat',Arial,sans-serif; margin-top:5vh" cellpadding="0"
                                            ...
                                            <tr>
                                                <td style="mso-padding-alt: 16px 24px; --bg-opacity: 1; background-color: #F05E7D; border-radius: 4px; font-family: Montserrat, -apple-system, 'Segoe UI', sans-serif;"
                                                    ...
                                                    <a target="_blank" href="{{ env('APP_URL') }}/{{ session('language') }}/admin/purchase-order-payment/{{ $slug }}"
                                                       style="display: block; font-weight: 600; font-size: 14px; line-height: 100%; padding: 16px 24px; --text-opacity: 1; color: #ffffff !important;; text-decoration: none;">{{ __('backend/email/purchase-order-send-customer.titulo_realizar_pago') }} &rarr;</a>
                                                </td>
                                            </tr>
                                        </table>
                                        <p style="font-size: 14px; line-height: 24px; margin-top: 5vh; margin-bottom: 20px;">
                                            {{ __('backend/email/purchase-order-send-customer.titulo_si_tiene_alguna_pregunta') }}
                                            <a target="_blank" href="#" style="color: #F05E7D">{{ __('backend/email/purchase-order-send-customer.titulo_soporte') }}
                                                {{ config('app.name') }}</a>{{ __('backend/email/purchase-order-send-customer.titulo_y_con_gusto') }}
                                        </p>
