                ...
                <div class="body">
                    <form class="pt-2 pr-4 pl-4 pb-4" method="post">
                        <form-wizard
                            ...
                            subtitle
                            title
                            finishButtonText="Crear Proveedor"
                            nextButtonText="Siguiente"
                            backButtonText="Atrás"
                            shape="tab"
                            error-color="#ff4949"
                            @on-change="cambioPagina"
                            @on-complete="createNewEntityProvider()"
                            ref="wizard"
                        >
                            <!------------------------------
                            ...
                            >
                                ...
                                example: $t('backend.customer.create-customers.titulo_telefono_ejemplo')
                                    ...
                                    <div class="row" v-if="addArchive">
                                        <div class="col-12">
                                            <label class="form-control-label" id="add-archive-dropzone-provider">{{ $t('backend.customer.create-customers.title_label_agregar_archivos') }}</label>
                                            <vue2Dropzone class="dropzone upload-logo dropzone-area dz-clickable"
                                                          ref="myVueDropzone"
                            ...
                            <template slot="footer" slot-scope="props">
                                ...
                                <div class="wizard-footer-right">
                                    ...
                                    <wizard-button v-else @click.native="createNewEntityProvider()"
...
export default {
    ...
    data() {
        return {
            ...
            errors: {},
            dropzoneOptions: {
                url: '/api/upload-picture-company-provider',
                // thumbnailWidth: 200,
                maxFilesize: 1,
                maxFiles: 20,
                paramName: 'picture',
            ...
            dropzoneOptionsCompanyArchive: {
                url: '/api/upload-archive-company-provider',
                // thumbnailWidth: 200,
                maxFilesize: 1,
                maxFiles: 20,
                paramName: 'archive',
    ...
    methods: {
            ...
            setTimeout(() => {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        axios.post('/api/register/store-company-provider', data).then(res => {
        ...
        removedArchiveDropzoneCompanyCustomer(file, error, xhr) {
            ...
            for (let i = 0; i < this.urlsArchiveCompany.length; i++)
                if (this.urlsArchiveCompany[i].uuid === uuid) {
                    ...
                    axios.post('/api/removed-archive-company-provider', data)
                        .then(resp => {
                            this.$toast.success({
                                title: this.$t('backend.customer.create-customers.title_muy_bien_toast'),
        ...
        removedImage() {
            ...
            axios.post('/api/removed-picture-company-provider', data)
                .then(resp => {
                    this.$toast.success({
                        title: this.$t('backend.customer.create-customers.title_muy_bien_toast'),
                        message: this.$t('backend.customer.create-customers.title_mensaje_success_toast_remove_image'),
        ...
        btnAddArchivesCompany(){
            if (this.companyName){
                ...
                this.eventSelectScroll('#add-archive-dropzone-provider')
            }else{
                this.$toast.error({
                    title: this.$t('backend.customer.create-customers.title_atención_toast'),
